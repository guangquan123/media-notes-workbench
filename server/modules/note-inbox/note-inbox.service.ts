...
export class NoteInboxService implements OnModuleInit, OnModuleDestroy {
  ...
  private async recordMessage(binding: typeof noteInboxBindings.$inferSelect, message: { messageId: string; content: string }): Promise<void> {
    ...
    const mediaRows = await this.db.insert(noteInboxMedia).values({ ownerId: binding.ownerId, platform: link.platform, canonicalKey: link.canonicalKey, canonicalUrl: link.canonicalUrl, noteStyle: binding.noteStyle, status: 'QUEUED', jobId: randomUUID(), attemptCount: 1 }).onConflictDoNothing().returning();
    ...
    if (!duplicate && media.jobId) { try { await this.noteJobsService.createFromInbox({ noteStyle: binding.noteStyle as NoteStyle, sourcePlatform: link.platform, sourceType: 'platform', url }, binding.ownerId, binding.larkUserId); await this.db.update(noteInboxMedia).set({ status: 'PROCESSING', updatedAt: new Date() }).where(eq(noteInboxMedia.id, media.id)); } catch (error) { await this.db.update(noteInboxMedia).set({ status: 'FAILED', lastError: error instanceof Error ? error.message : '任务创建失败', updatedAt: new Date() }).where(eq(noteInboxMedia.id, media.id)); } }
  }
