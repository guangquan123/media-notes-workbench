...
启动器会通过用户级 `launchd` 在后台启动本地服务，待页面可访问后自动打开应用；启动后可以关闭该终端窗口，服务异常退出也会自动重启。
启动日志写入 `logs/dev-local-launch.log`，服务运行日志写入 `logs/dev.std.log`。
...
它会先卸载本项目的后台守护，再停止由本项目启动器记录的开发进程，不会影响其他项目。
双击 [重启多媒体笔记工作台.command](重启多媒体笔记工作台.command) 会原子重启后台服务，并在页面就绪后自动打开首页。
