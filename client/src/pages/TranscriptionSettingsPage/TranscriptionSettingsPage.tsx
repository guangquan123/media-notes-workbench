...
import type { TencentAsrQuotaStatus, TencentAsrSettings } from '@shared/api.interface';
import {
...
export default function TranscriptionSettingsPage({
  ...
  const checkQuota = async (): Promise<void> => {
    ...
    try {
      const next: TencentAsrQuotaStatus = await getTencentAsrQuota();
      setQuota(next);
      ...
      toast.error('额度查询失败，请检查腾讯云配置与权限');
    } finally {
