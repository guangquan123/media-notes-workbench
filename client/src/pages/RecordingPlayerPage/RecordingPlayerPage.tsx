...
export default function RecordingPlayerPage() {
  ...
  const saveTitle = async (): Promise<void> => {
    ...
    setBusy(true);
    try {
      const response = await updateRecordingAsset(selected.id, { title: renameTitle.trim() });
      setAssets((current) => current.map((asset) => asset.id === response.item.id ? response.item : asset));
      setRenameOpen(false);
      toast.success('标题已更新');
    } catch (error: unknown) {
  ...
  return <main className="recording-player-page min-h-screen bg-[#111b32] text-white">
    ...
    </div><Dialog onOpenChange={setRenameOpen} open={renameOpen}><DialogContent><DialogHeader><DialogTitle>修改录音标题</DialogTitle><DialogDescription>标题将同步更新到录音记录。</DialogDescription></DialogHeader><Input autoFocus onChange={(event) => setRenameTitle(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void saveTitle(); }} value={renameTitle} /><DialogFooter><Button onClick={() => setRenameOpen(false)} variant="outline">取消</Button><Button disabled={busy || !renameTitle.trim()} onClick={() => void saveTitle()}>保存</Button></DialogFooter></DialogContent></Dialog></main>;
}
