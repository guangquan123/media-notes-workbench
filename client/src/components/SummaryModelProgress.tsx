...
export function SummaryModelProgress({ job }: SummaryModelProgressProps) {
  ...
  const Icon = summary.stage === 'completed'
    ? CheckCircle2
    ...
    : summary.provider === 'external_model'
        ? BrainCircuit
        : Sparkles;
  return <div className="mt-5 rounded-xl border border-violet-200 bg-violet-50 p-4 text-sm text-violet-950"><div className="flex items-center gap-2 font-semibold"><Icon className="size-4 text-violet-700" />总结模型：{summary.modelName}</div><p className="mt-1 text-xs leading-5 text-violet-900/75">{STAGE_COPY[summary.stage]}。这里展示的是可验证的处理阶段，不包含模型内部推理内容。</p></div>;
}
